    ...
    class image{
        ...
        public function Insert_Image(){
            ...
            for($i = 0; i < $num_images; $i++){
                $query = "CALL image_SP(0, '$this->image[$i]', 0, 1, 'INSERT')";
        ...
        public function Update_Image(){
            $this->objConection->conexion();
            $query = "CALL image_SP($this->id_image, '$this->image', $this->cover, 0, 'UPDATE')";
            $resultado = $this->objConection->cone->query($query);
            if($resultado){
				$this->objConection->disconnect();
				return true; 
        ...
        public function Last_Inserted(){
            $this->objConection->conexion();
			$query = "CALL image_SP(0, null, 0, 0, 'LAST_INSERTED')";
			$resultado = $this->objConection->cone->query($query);

			$fila = $resultado->fetch_assoc();
			if($resultado->num_rows > 0){

                $this->id_image = $fila['id_News'];
